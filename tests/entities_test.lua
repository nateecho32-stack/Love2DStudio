...
T.case("entities: schema'd string props survive validate; unknown props drop", function()
  local E = makeWorld()
  local id = E:spawn("goblin", 0, 0, { name = "gordon", headwear = "beret" })
  local arch = E.ecs:get(id, "archetype")
  T.eq(arch.props.name, "gordon", "the inspector's name field must reach gameplay")
  T.eq(arch.props.headwear, nil, "props outside the schema stay dropped")
end)
